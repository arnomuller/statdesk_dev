...
as.data.frame(filter_data2 %>% 
                ...
                ungroup() %>% 
                rows_insert(tibble(Variables = "Total", Variable2 = "Total", Pct_c = NA)) %>% 
                complete(Variables,Variable2)%>% 
                group_by(Variable2) %>% 
                mutate(Pct_c = ifelse(Variables == "Total", sum(Pct_c, na.rm = T), Pct_c)) %>% 
                group_by(Variables) %>% 
                mutate(Pct_c = round(ifelse(Variable2 == "Total", 100*sum(Effectif, na.rm = T)/nrow(filter_data2), Pct_c),2))%>% 
